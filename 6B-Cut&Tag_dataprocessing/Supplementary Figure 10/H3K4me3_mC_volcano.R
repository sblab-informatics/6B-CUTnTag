...
# ---- Plot ----
# Note: no layer-level rasterisation here.
# The entire plot is rasterised only at export (see ggsave below).
p <- ggplot(df, aes(x = meth.diff, y = -log10(pvalue))) +
  geom_vline(xintercept = c(-0.6, 0.6), col = "grey", linetype = "dashed") +
  geom_hline(yintercept = -log10(0.05), col = "grey", linetype = "dashed") +

  # Background points (vector in ggplot; rasterised at export)
  geom_point(
    aes(color = Sig),
    size = bg_size,
    alpha = 0.9
  ...
  scale_color_manual(values = c(
    "Increased" = "darkorange2",
    "Decreased" = "darkorange2",
    "No"        = "grey"
  )) +
  ...
  ggtitle("H3K4Me3 vs WG mC")

# ---- Save PDF ----
# Rasterise EVERYTHING at the very end:
# this wraps the complete ggplot into a single raster layer embedded in the PDF.
ggsave(
  "H3K4Me3_all_DMCs.mC.pdf",
